...
def activePos(flag):
    ...
    if flag == 1:
        phiActive = 0
        alphaActive = np.pi # Active angle for motor 2 (pitch)
        thetaActive = 0
    else:
        phiActive = 0 # Idle angle for motor 1
        alphaActive = np.pi/2 # Idle angle for motor 2
        thetaActive = 3*np.pi/2 # Idle angle for motor 3
