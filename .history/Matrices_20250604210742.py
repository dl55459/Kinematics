...
"""
def inverse(T_BEE_subs, LM1M3, LM3EE, xDesired, yDesired):
...
def activePos(flag):
    ...
    if flag == 1:
        alphaActive = 0 # Angle for motor 2 (pitch)
    else:
        phiActive = 0 # Angle for motor 1
        alphaActive = pi/2 # Angle for motor 2
        thetaActive = 3*pi/2 # Angle for motor 3
...
phiD, alphaD, thetaD = activePos(False) # Set arm to idle position
...
phiD, alphaD, thetaD = activePos(True) # Set arm to active position